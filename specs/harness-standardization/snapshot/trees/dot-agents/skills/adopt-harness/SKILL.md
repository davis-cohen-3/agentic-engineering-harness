...
```
<harness-path>/.Codex/skills/adopt-harness/copy.sh . [python|ts|none]
```

`copy.sh` is self-locating (it finds the harness from its own path) and owns the manifest
(the definition of what travels). It never clobbers an existing `AGENTS.md` or `make/gate.mk`
...
- Confirm the copied tree is clean: no `docs/`, no `recommended/` catalogs, no
  `adopt-harness/`; `AGENTS.md` is the filled profile; `.Codex/FLOOR.md` byte-identical to base.
